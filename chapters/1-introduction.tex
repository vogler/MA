...
% floats, OSEK, OpIAT
Within the scope of the OpIAT project \cite{opiat}, Goblint is used for interprocedural analysis of programs with dynamic thread-creation, especially value analysis in OSEK\footnote{OSEK-OS is a real-time operating system for embedded components, mainly for automotive applications.} programs, for which the handling of floats would be useful.

There is also an Eclipse plug-in that displays the results of the analysis in a view and adds warning markers for places in the code where a data race might occur. A screenshot can be seen in \refFigure{screenshot_eclipse}.

If you want to try Goblint, there is a getting started guide in the appendix (\refChapter{app:setup}).
